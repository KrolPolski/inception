...
# The first time we run this, configure the server to be reachable
...
GRANT ALL PRIVILEGES ON *.* to 'root'@'%' IDENTIFIED BY '$MYSQLROOT_PASSWORD';
FLUSH PRIVILEGES;
EOF

	# Shut downt he temporary server and mark the volume as initialized
	mysqladmin shutdown
